...
void main() {
  ...
  test(
    'Initial state',
    () => expect(
      ItemsAscensionStatsBloc(genshinService).state,
      const ItemsAscensionStatsState.loading(),
      reason: 'A fresh ItemsAscensionStatsBloc should start in loading state',
    ),
  );
  ...
  group('Init', () {
    blocTest<ItemsAscensionStatsBloc, ItemsAscensionStatsState>(
      ...
      build: () => ItemsAscensionStatsBloc(genshinService),
      act: (bloc) => bloc.add(ItemsAscensionStatsEvent.init(itemType: ItemType.character, type: characterPossibleStats.first)),
      expect: () {
        final items = genshinService.getItemsAscensionStats(characterPossibleStats.first, ItemType.character);
    ...
    blocTest<ItemsAscensionStatsBloc, ItemsAscensionStatsState>(
      ...
      expect: () {
        final items = genshinService.getItemsAscensionStats(weaponPossibleStats.first, ItemType.weapon);
